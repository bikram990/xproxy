...
void Session::OnClientDataReceived(const boost::system::error_code& e) {
    /// when client timeout is triggered, there is still read operation on this
    /// socket, close the socket will lead to "operation_aborted" error
    /// so we check here to determine if the "operation_aborted" error is
    /// generated by socket closing, if it is, we just return
...
void Session::OnClientTimeout(const boost::system::error_code& e) {
    if(e == boost::asio::error::operation_aborted)
        ...
        // TODO add logic here
        client_timer_triggered_ = true;
        manager_.stop(shared_from_this());
    }
}

void Session::InitClientSSLContext() {
